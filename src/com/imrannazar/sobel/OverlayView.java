...
{
    private static final String LOGTAG = "Sobel";
    private Camera mCam;
    ...
    public void setPostMortemReportExceptionHandler(PostMortemReportExceptionHandler p)
    {
	pmeh = p;
    }
	...
	mCam.setPreviewCallback(new PreviewCallback()
	{
		...
		{
//			Log.v(LOGTAG, "Preview input doesn't match frame size; skipping");
		...
		if(mRunning && cOver!=null)
		{
		    try
		    {
//	    		Log.v(LOGTAG, "Preview input: "+f.length+" bytes; frame copy: "+mFrame.length+" bytes");
			pmeh.preErrStr += "Preview input: "+f.length+" bytes; frame copy: "+mFrame.length+" bytes\n";
			System.arraycopy(f, 0, mFrame, 0, mFrame.length);
			doNativeProcessing(mFrame, mFrameSize.width, mFrameSize.height, mFrameDiff);
			mFrameDiff.position(0);

			Bitmap bmp = Bitmap.createBitmap(mFrameSize.width>>0, mFrameSize.height>>0, Bitmap.Config.ARGB_8888);
			bmp.copyPixelsFromBuffer(mFrameDiff);
			Rect src = new Rect(0, 0, (mFrameSize.width>>0)-1, (mFrameSize.height>>0)-1);
			RectF dst = new RectF(0, 0, cOver.getWidth()-1, cOver.getHeight()-1);
